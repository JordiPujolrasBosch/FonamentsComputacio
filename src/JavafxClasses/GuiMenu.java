...
public class GuiMenu {
    ...
    static class GuiMenuIn {
        ...
        public CallPane getCallPane() {
            ...
            switch (type){
                ...
                case L2 -> cp = () -> {
                    Layout2 l = new Layout2(title, description, a, b, callMenuOne);
                    return l.build();
                };
                case L2W -> cp = () -> {
                    Layout2Words l = new Layout2Words(title, description, a, b, callMenuOne);
                    return l.build();
                };
                case L2X1 -> cp = () -> {
                    Layout2x1 l = new Layout2x1(title, description, a, b, c, callMenuTwo);
                    return l.build();
                };
                case L3 -> cp = () -> {
                    Layout3 l = new Layout3(title, description, a, b, c, callMenuTwo);
                    return l.build();
                };
                case L3W -> cp = () -> {
                    Layout3Words l = new Layout3Words(title, description, a, b, c, callMenuTwo);
                    return l.build();
                };
            }
            return cp;
    ...
    private static GuiMenuIn buildFindCounterExampleCfg(){
        ...
        String description = "Check if two context free grammars are equivalent.\nIf are different it search a counterexample,\na word that can be generated by only one of them.";
        CallMenuTwo call = (a, b) -> {
            try{
                Cfg aa = Reader.readGrammarString(a);
                Cfg bb = Reader.readGrammarString(b);
                return Menu.findCounterExampleCfg(aa,bb,null,null);
            }
            catch (Exception ex){
                return ex.getMessage();
            }
    ...
    private static GuiMenuIn buildFindManyCounterExamplesCfg(){
        ...
        String description = "Check if two context free grammars are equivalent.\nIf are different it search some counterexamples,\nwords that can be generated by only one of them.";
        CallMenuTwo call = (a, b) -> {
            try{
                Cfg aa = Reader.readGrammarString(a);
                Cfg bb = Reader.readGrammarString(b);
    ...
    private static GuiMenuIn buildSimplifyCfg(){
        ...
        String description = "Simplify a context free grammar.\nRemoves non derivable rules, non reachable rules, empty rules and unit rules.";
        CallMenuOne call = data -> {
            try {
                Cfg cfg = Reader.readGrammarString(data);
    ...
    private static GuiMenuIn buildGriebachCfg(){
        String title = "Transform cfg griebach";
        String description = "Transform a context free grammar to the griebach form.\nOn griebach form, every derivation starts with a terminal.";
        CallMenuOne call = data -> {
            try {
                Cfg cfg = Reader.readGrammarString(data);
                return Menu.griebachCfg(cfg);
            }
            catch (Exception ex){
                return ex.getMessage();
            }
